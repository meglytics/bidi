...
case class Connectors(source: String, endpoint: String, port: String, dbname: String, inputs: models.analytics.KeyValueList, tables: TablesList) {
  val json = "{\"source\":\"" + source + "\",\"endpoint\":\"" + endpoint + "\",\"port\":\"" + port + "\",\"dbname\":\"" + dbname + "\",\"inputs\":" + models.analytics.KeyValueList.toJson(inputs, true) + ",\"tables\":" + TablesList.toJson(tables, true) + "}"
...
object Connectors {
  def empty: Connectors = new Connectors(new String(), new String(), new String(), new String(), models.analytics.KeyValueList.empty, TablesList.empty)
...
case class Tables(name: String, table_id: String, schemas: models.analytics.KeyValueList, links: models.analytics.KeyValueList) {
  val json = "{\"name\":\"" + name + "\",\"table_id\":\"" + table_id + "\",\"schemas\":" + models.analytics.KeyValueList.toJson(schemas, true) + ",\"links\":" + models.analytics.KeyValueList.toJson(links, true) + "}"
...
object Tables {
  def empty: Tables = new Tables(new String(), new String(), models.analytics.KeyValueList.empty, models.analytics.KeyValueList.empty)
...
object Workbenches {
  ...
  private def mkGunnySack(email: String, input: String): ValidationNel[Throwable, Option[GunnySack]] = {
    ...
    for {
      ...
      uir <- (UID("wob").get leftMap { ut: NonEmptyList[Throwable] => ut })
    } yield {
